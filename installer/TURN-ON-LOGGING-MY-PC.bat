@echo off
REM ============================================================================
REM  ScanBridge - turn on detailed logging ON THIS PC.
REM
...
title ScanBridge - Detailed Logging
color 0B
...
reg query HKCU\SOFTWARE\ScanBridge /v LogLevel 2>nul | find /i "Debug" >nul
if not errorlevel 1 goto :turnoff

reg add HKCU\SOFTWARE\ScanBridge /v LogLevel /t REG_SZ /d Debug /f >nul
if errorlevel 1 (
...
echo     2. Right-click the ScanBridge icon near the clock, choose Exit.
echo     3. Start ScanBridge again from the Start Menu.
...
:turnoff
reg add HKCU\SOFTWARE\ScanBridge /v LogLevel /t REG_SZ /d Information /f >nul
echo   Detailed logging is now OFF (back to normal).
echo.
echo   Close and reopen Remote Desktop Connection and ScanBridge
