@echo off
REM ============================================================================
REM  ScanBridge - allow a Remote Desktop server to reach this PC directly.
REM
REM  RUN THIS AS ADMINISTRATOR (right-click, "Run as administrator").
REM
REM  This is the one part of ScanBridge that needs administrator rights,
...
REM  your local network only, to the ScanBridge program only. Anything that
...
setlocal
title ScanBridge - Allow Direct Connection
color 0B
...
set "RULE=ScanBridge (direct connection)"
set "PORT=47214"
set "TARGET=%LOCALAPPDATA%\Programs\ScanBridge\ScanBridge.Client.exe"

REM %LOCALAPPDATA% belongs to the administrator when elevated, not to the person who installed
REM ScanBridge, so the per-user install path is resolved from the original profile instead.
if not exist "%TARGET%" (
    ...
    if defined OWNER set "TARGET=C:\Users\%OWNER%\AppData\Local\Programs\ScanBridge\ScanBridge.Client.exe"
)
...
echo   and enter the code from the Pairing Code button in ScanBridge.
echo.
pause
exit /b 0
