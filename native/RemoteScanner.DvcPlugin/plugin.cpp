// plugin.cpp — RemoteScanner RDP Dynamic Virtual Channel plugin.
//
...
//
// Its only job is to be a byte pump: it owns the client end of the "RemoteScanner" dynamic
    ...
    bool start() {
        try {
            ...
            std::vector<uint8_t> secret =
                loadProtectedSecretOnCleanThread(L"Software\\RemoteScanner", L"Secret");
...
BOOL APIENTRY DllMain(HMODULE module, DWORD reason, LPVOID) {
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        RS_LOG_INFO("RemoteScanner DVC plugin loaded into pid %lu", GetCurrentProcessId());
    }
    return TRUE;
}
