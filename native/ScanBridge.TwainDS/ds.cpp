// ds.cpp — ScanBridge virtual TWAIN Data Source.
//
// This DLL is installed as C:\Windows\twain_32\ScanBridge\ScanBridge.ds (and the
// twain_64 equivalent). Any TWAIN application on the RDP server enumerates it as a normal
// scanner named "ScanBridge (<client PC>)". Every TWAIN operation is proxied over a
...
std::wstring sessionRegistryKey() {
    return L"Software\\ScanBridge\\Session\\" + std::to_wstring(currentSessionId());
}
...
std::string productName() {
    ...
    if (client.empty()) return "ScanBridge";

    // The closing bracket is reserved before the name is trimmed, so a long or non-Latin
    // machine name yields "ScanBridge (LONGNAME…)" rather than a sentence that stops dead.
    constexpr size_t kLimit = sizeof(TW_STR32) - 2;
    const std::string prefix = "ScanBridge (";
...
bool DataSource::connectToAgent() {
    try {
        ...
        const std::wstring pipeName =
            L"ScanBridge.Session." + sid + L"." + std::to_wstring(sessionId);
        ...
        fail(TWCC_CHECKDEVICEONLINE,
             std::string("cannot reach the ScanBridge agent: ") + ex.what());
        return false;
    }
            ...
            case DAT_IDENTITY:
                switch (msg) {
                    case MSG_GET: {
                        ...
                        setStr32(identity->Manufacturer, "ScanBridge");
                        setStr32(identity->ProductFamily, "ScanBridge");
...
BOOL APIENTRY DllMain(HMODULE module, DWORD reason, LPVOID) {
    switch (reason) {
        case DLL_PROCESS_ATTACH:
            ...
            RS_LOG_INFO("ScanBridge data source loaded into pid %lu (%u-bit)",
                        GetCurrentProcessId(), static_cast<unsigned>(sizeof(void*) * 8));
            break;
        case DLL_PROCESS_DETACH:
            RS_LOG_INFO("ScanBridge data source unloading");
            break;
        default:
            break;
    }
