...
{
    public const string ServiceName = "RemoteScanner";
    private const string DisplayName = "Remote Scanner Redirection";
    ...
    {
        (Path.Combine(Environment.SystemDirectory, "..", "twain_64", "RemoteScanner"), "x64/RemoteScanner.ds"),
        (Path.Combine(Environment.SystemDirectory, "..", "twain_32", "RemoteScanner"), "x86/RemoteScanner.ds"),
    };
    ...
    public static int Install(string? installDirectory)
    {
        ...
        try
        {
            ...
            string installedExe = Path.Combine(target, "RemoteScanner.Service.exe");
            CopySelf(installedExe);

            foreach ((string directory, string resource) in TwainTargets())
            {
                string full = Path.GetFullPath(directory);
                string file = Path.Combine(full, "RemoteScanner.ds");
            ...
            using (RegistryKey key = Registry.LocalMachine.CreateSubKey(@"SOFTWARE\RemoteScanner"))
            ...
            Console.WriteLine("named 'Remote Scanner (<the client PC>)' once a client connects with the");
    ...
    public static int Uninstall()
    {
        ...
        try
        {
            Registry.LocalMachine.DeleteSubKeyTree(@"SOFTWARE\RemoteScanner", throwOnMissingSubKey: false);
        }
        catch (Exception ex)
        {
    ...
    {
        foreach (Process process in Process.GetProcessesByName("RemoteScanner.Service"))
        {
    ...
    {
        string stale = Path.Combine(Path.GetDirectoryName(vendorDirectory)!, "RemoteScanner.ds");
