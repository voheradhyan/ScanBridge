...
///   HKCU\Software\RemoteScanner                       Secret     (local PC: agent &lt;-&gt; DVC plugin)
///   HKCU\Software\RemoteScanner\Session\&lt;sessionId&gt;   Secret     (server: session agent &lt;-&gt; .ds)
///                                                     ClientName (name shown in the scanner list)
/// </summary>
[SupportedOSPlatform("windows")]
public static class SecretStore
{
    private const string LocalKeyPath = @"Software\RemoteScanner";
...
public static class Dpapi
{
    ...
    private static unsafe byte[] Transform(ReadOnlySpan<byte> input, bool protect)
    {
        ...
        try
        {
            ...
            bool ok = protect
                ? Win32.CryptProtectData(ref inBlob, "RemoteScanner", IntPtr.Zero, IntPtr.Zero,
