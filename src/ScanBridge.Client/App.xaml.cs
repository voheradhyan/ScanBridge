...
public partial class App : System.Windows.Application
{
    ...
    /// </summary>
    private const string SingleInstanceName = @"Local\ScanBridge.Agent.SingleInstance";
    ...
    /// </summary>
    private const string ShowWindowEventName = @"Local\ScanBridge.Agent.ShowWindow";
    ...
    private static int RunCommandLine(string[] args)
    {
        ...
        return role switch
        {
            ...
            _ => ScanBridge.Agent.Program.RunAsync(args).GetAwaiter().GetResult(),
        };
    }
    ...
    {
        Console.WriteLine($"""
            ScanBridge — the half that runs on the PC with the scanner.
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (!createdNew)
        {
            ...
            //
            // This used to show "ScanBridge is already running" and exit, which is true but
            ...
            catch (WaitHandleCannotBeOpenedException)
            {
                ...
                MessageBox.Show(
                    "ScanBridge is already running, but it is an older version that cannot " +
                    "be brought to the front.\n\n" +
                    "Right-click its icon near the clock (you may need the small arrow to see " +
                    "hidden icons) and choose Exit, then open ScanBridge again.",
                    "ScanBridge", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (UnauthorizedAccessException)
            {
                MessageBox.Show(
                    "ScanBridge is already running under a different account.",
                    "ScanBridge", MessageBoxButton.OK, MessageBoxImage.Information);
            }
        ...
        try
        {
            ...
            {
                MessageBox.Show(
                    "ScanBridge.ScanHost.exe was not found.\n\n" +
        ...
        catch (Exception ex)
        {
            ...
            MessageBox.Show($"ScanBridge could not start:\n\n{ex.Message}",
                            "ScanBridge", MessageBoxButton.OK, MessageBoxImage.Error);
            Shutdown(1);
        }
    }
    ...
    private void WarnIfPolicyBlocksTheAddIn()
    {
        ...
        _trayIcon?.ShowBalloonTip(10_000, "ScanBridge", reason, Forms.ToolTipIcon.Warning);
    }
