...
/// ScanBridge.Server when the session connects.
///
...
public static class Program
{
    ...
    /// <c>ScanBridge.Server.exe --session-agent</c>. Not a Main: this lives in the server
    ...
    private static void StopOrphanedAgents(uint sessionId)
    {
        ...
        foreach (Process other in Process.GetProcessesByName("ScanBridge.SessionAgent"))
        {
            using (other)
            {
                try
